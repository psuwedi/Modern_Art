...
function _bsa_optimize_refresh (id) {
	...
	window.googletag.cmd.push(
		function() {
			window.purchs2s.cmd.push(
				function() {
					var slot = id.slice(0, -2);
					
					if(typeof placements[slot] != 'undefined')
					{
						var slot_def = window.googletag.defineSlot('/' + network + '/' + placements[slot].name, placements[slot].sizes, id)
							.defineSizeMapping(
								window.purchs2s.sizeMapping(placements[slot].id)
									.addSize(placements[slot].options.sizemap[1][0], placements[slot].options.sizemap[1][1])
									.build()
							)
							.addService(window.googletag.pubads());
						
						window.googletag.display(id);
						window.googletag.pubads().refresh([slot_def]);
					}
				}
			);
	 	}
